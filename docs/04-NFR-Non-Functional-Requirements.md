...
Quantifies *how well* the system shall perform, not what it does. Every NFR here is **measurable** and has a stated **fit-criterion** (pass/fail) and **verification method**.
...
*Measurement*: from Cloudflare/Vercel edge timing (`x-vercel-cache`, server-timing header if added) or `pino` request duration field.
...
*Next: `05-Use-Cases-and-User-Stories.md` maps these NFRs to user-visible scenarios with step-by-step flows.*
