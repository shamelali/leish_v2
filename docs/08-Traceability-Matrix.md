...
| `referrals` table + `referral_code` columns | BRS §4.2 deferred (referral MVP scaffold only) — no business rule yet; code is *pre-requirement scaffolding* for v2.2 | **Deferred — flag red if exposed before BR**; tests exist but feature hidden |
...
*This RTM is the single reference for reviewers (security, QA, compliance) to confirm every promised capability is implemented and tested.*
