...
This SRS translates the business requirements (BRS `01`) into verifiable _software_ requirements. Its audience is architects, developers, QA, and auditors. It is the single reference for what the system **shall** do, under what constraints, and how compliance will be shown.
...
| **Billplz API v3**   | HTTPS POST `application/x-www-form-urlencoded`, Basic Auth | `src/server/payments.ts`                                                     | `BILLPLZ_API_KEY` (also the webhook HMAC secret), `BILLPLZ_COLLECTION_ID`, `BILLPLZ_BASE_URL`, `BILLPLZ_CALLBACK_URL` |
...
_Each feature lists its SRS requirements. Full functional decomposition with acceptance criteria lives in FRS (`03`)._
...
_Next: `03-FRS-Functional-Requirements-Specification.md` details every FR with actor, pre/post conditions, flow, and acceptance criteria._
