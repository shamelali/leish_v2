...
| GitHub Actions | ❌ **billing-locked** (re-confirmed 2026-08-19 11:40Z — the readiness PR's `check`/`e2e` jobs also died in ~2s). Both jobs on `main` failed instantly: *"The job was not started because your account is locked due to a billing issue."* No green e2e run exists in recorded history. **Owner must clear the billing lock.** |
