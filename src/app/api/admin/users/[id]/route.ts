...
export const PATCH = statefulRoute(async function PATCH(
  request: Request,
  { params }: { params: Promise<{ id: string }> },
) {
  ...
  await db.prepare(`UPDATE users SET ${updates.join(", ")} WHERE id = @id`).run({ ...bindParams, id });
  ...
  await logAdminAction(admin.id, "update_user", "users", id, {
    fields: Object.keys(body.data).filter((k) => body.data[k as keyof typeof body.data] !== undefined),
  });

  return NextResponse.json({ user: toPublicUser(updated) });
}, { route: "PATCH /api/admin/users/[id]" });

export const DELETE = statefulRoute(async function DELETE(
  _request: Request,
  { params }: { params: Promise<{ id: string }> },
) {
  ...
  const existing = await db.prepare("SELECT id FROM users WHERE id = ?").get<{ id: string }>(id);
  if (!existing) {
    return jsonError("User not found", 404);
  }
