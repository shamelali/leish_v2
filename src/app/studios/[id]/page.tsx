  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
      <nav className="text-sm text-stone-500 dark:text-stone-400">
        <Link href="/studios" className="hover:text-rose-600 dark:hover:text-rose-400">Studios</Link>
      ...
      <div className="mt-6 grid gap-8 lg:grid-cols-[1fr_360px]">
        <div>
          <div className="relative aspect-[16/9] overflow-hidden rounded-3xl bg-stone-100 dark:bg-stone-800">
            <Image src={studio.image} alt={studio.name} fill priority sizes="(max-width: 1024px) 100vw, 60vw" className="object-cover" />
          </div>

          <h1 className="mt-6 font-display text-4xl font-semibold tracking-tight text-stone-900 dark:text-stone-100">{studio.name}</h1>
          ...
          <div className="mt-8">
            <h2 className="font-display text-2xl font-semibold text-stone-900 dark:text-stone-100">About the studio</h2>
            <p className="mt-3 leading-7 text-stone-600 dark:text-stone-400">{studio.description}</p>
          </div>

          <div className="mt-8">
            <h2 className="font-display text-2xl font-semibold text-stone-900 dark:text-stone-100">Services</h2>
            <div className="mt-4 flex flex-wrap gap-2">
              {studio.services.map((s) => (
                <span key={s} className="rounded-full border border-rose-200 bg-rose-50 px-3 py-1.5 text-sm font-medium text-rose-700 dark:border-rose-800/60 dark:bg-rose-500/10 dark:text-rose-400">
                  {s}
                </span>
              ))}
            </div>
        ...
        <aside className="h-fit rounded-2xl border border-stone-200 bg-white p-6 dark:border-stone-800 dark:bg-stone-900">
          ...
          <p className="font-display text-3xl font-semibold text-stone-900 dark:text-stone-100">{formatRM(studio.priceFrom)}</p>
          <dl className="mt-5 space-y-3 border-t border-stone-100 pt-5 text-sm dark:border-stone-800">
            <div className="flex justify-between gap-4">
              ...
              <dd className="text-right font-medium text-stone-900 dark:text-stone-100">{studio.hours}</dd>
            </div>
            <div className="flex justify-between gap-4">
            ...
            <div className="flex justify-between gap-4">
              ...
              <dd className="text-right font-medium text-stone-900 dark:text-stone-100">{studio.area}, {studio.state}</dd>
            </div>
          </dl>
          <div className="mt-6">
            <Button href="/register" className="w-full">Book an appointment</Button>
          </div>
          <p className="mt-3 text-center text-xs text-stone-400 dark:text-stone-500">Demo — appointments require a free account</p>
        </aside>
      </div>
    </div>
  );
