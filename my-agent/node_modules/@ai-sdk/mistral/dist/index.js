...
var mistralPromptTokensDetailsSchema = z.object({
  messages: z.array(z.json()).nullish(),
  cached_tokens: z.number().nullish(),
  audio_tokens: z.number().nullish()
}).catchall(z.json());
var mistralCompletionTokensDetailsSchema = z.object({
...
var mistralLanguageModelChatOptions = z2.object({
  /**
   * Whether to inject a safety prompt before all conversations.
   *
   * Defaults to `false`.
   */
  safePrompt: z2.boolean().optional(),
  documentImageLimit: z2.number().optional(),
  documentPageLimit: z2.number().optional(),
  /**
   ...
   */
  reasoningEffort: z2.enum(["high", "none"]).optional()
});

// src/mistral-error.ts
import { createJsonErrorResponseHandler } from "@ai-sdk/provider-utils";
import { z as z3 } from "zod/v4";
var mistralErrorDataSchema = z3.object({
  object: z3.literal("error"),
  message: z3.string(),
  type: z3.string(),
  param: z3.string().nullable(),
...
var mistralContentSchema = z4.union([
  z4.string(),
  z4.array(
    z4.discriminatedUnion("type", [
      ...
      z4.object({
        type: z4.literal("reference"),
        reference_ids: z4.array(z4.union([z4.string(), z4.number()]))
      }),
      z4.object({
...
var mistralChatResponseSchema = z4.object({
    ...
    z4.object({
      message: z4.object({
        ...
        tool_calls: z4.array(
          z4.object({
            id: z4.string(),
            function: z4.object({ name: z4.string(), arguments: z4.string() })
          })
        ).nullish()
      }),
      index: z4.number(),
  ...
  object: z4.literal("chat.completion"),
  usage: mistralUsageSchema
});
var mistralChatChunkSchema = z4.object({
  id: z4.string().nullish(),
  created: z4.number().nullish(),
  model: z4.string().nullish(),
  choices: z4.array(
    z4.object({
      delta: z4.object({
        ...
        tool_calls: z4.array(
          z4.object({
            index: z4.number().nullish(),
            id: z4.string().nullish(),
            function: z4.object({
              name: z4.string().nullish(),
              arguments: z4.string().nullish()
            })
          })
        ).nullish()
      }),
...
// src/mistral-embedding-model-options.ts
import { z as z5 } from "zod/v4";
var mistralEmbeddingModelOptions = z5.object({
  /**
   ...
   */
  outputDimension: z5.number().int().positive().optional(),
  /**
   * The data type of the output embeddings when supported by the model.
   */
  outputDtype: z5.enum(["float", "int8", "uint8", "binary", "ubinary"]).optional()
});
...
var MistralEmbeddingModel = class _MistralEmbeddingModel {
  ...
  async doEmbed({
    ...
    const mistralOptions = (_a = await parseProviderOptions2({
      provider: "mistral",
      providerOptions,
    ...
    const {
      ...
      headers: combineHeaders2((_c = (_b = this.config).headers) == null ? void 0 : _c.call(_b), headers),
      body: {
        model: this.modelId,
        input: values,
...
var MistralTextEmbeddingResponseSchema = z6.object({
...
var mistralSpeechModelOptions = z7.object({
  /**
   ...
   */
  refAudio: z7.string().min(1).optional()
});
...
var mistralSpeechResponseSchema = z8.object({
  audio_data: z8.string()
});
...
var mistralTranscriptionModelOptions = lazySchema(
  () => zodSchema(
    z9.object({
       ...
       * accuracy.
       */
      language: z9.string().min(1).optional(),
      /**
       ...
       */
      timestampGranularities: z9.array(z9.enum(["segment", "word"])).min(1).optional(),
      /**
       * Whether to identify speakers in the transcription.
       */
      diarize: z9.boolean().optional(),
      /**
       ...
       */
      contextBias: z9.array(
        z9.string().min(1).regex(
          /^[^\s,]+$/,
...
var mistralTranscriptionResponseSchema = z10.object({
  model: z10.string(),
  text: z10.string(),
  language: z10.string().nullish(),
  segments: z10.array(
    z10.object({
      type: z10.literal("transcription_segment").nullish(),
      text: z10.string(),
      start: z10.number(),
      end: z10.number(),
      score: z10.number().nullish(),
      speaker_id: z10.string().nullish()
    })
  ).nullish(),
  usage: z10.object({
    prompt_tokens: z10.number().nullish(),
    completion_tokens: z10.number().nullish(),
    total_tokens: z10.number().nullish(),
    prompt_audio_seconds: z10.number().nullish(),
    request_count: z10.number().nullish()
  }).nullish()
});
