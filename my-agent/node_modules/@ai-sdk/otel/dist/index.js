...
function sanitizeAttributeValue(value) {
  ...
  if (primitiveType === "number") {
    ...
    return numbers.every(Number.isFinite) ? numbers : void 0;
  }
...
var OpenTelemetry = class {
  ...
  onGenerateStart(event) {
    ...
    const attributes = selectAttributes(telemetry, {
      ...
      "gen_ai.system_instructions": event.instructions != null ? {
  ...
  onObjectStepStart(event) {
    ...
    const providerName = mapProviderName(event.provider);
    const systemInstructionCount = state.objectSystemInstructions == null ? 0 : formatSystemInstructions(state.objectSystemInstructions).length;
    const inputMessages = (_a = event.promptMessages) == null ? void 0 : _a.slice(systemInstructionCount);
    const attributes = selectAttributes(telemetry, {
      ...
      "gen_ai.request.temperature": (_b = state.settings.temperature) != null ? _b : void 0,
      "gen_ai.request.top_k": state.settings.topK,
      "gen_ai.request.top_p": state.settings.topP,
      "gen_ai.system_instructions": state.objectSystemInstructions != null ? {
        input: () => JSON.stringify(
          formatSystemInstructions(state.objectSystemInstructions)
        )
      } : void 0,
      "gen_ai.input.messages": {
        input: () => inputMessages ? JSON.stringify(formatInputMessages(inputMessages)) : void 0
      },
  ...
  onLanguageModelCallEnd(event) {
    ...
    if (!(state == null ? void 0 : state.inferenceSpan)) return;
    const { telemetry } = state;
    const toolDefinitions = [...(_a = state.inferenceToolDefinitions) != null ? _a : []];
    ...
    for (const part of event.content) {
      if (part.type !== "tool-call" || !part.providerExecuted || definedToolNames.has(part.toolName)) {
        continue;
      }
    ...
    state.inferenceSpan.setAttributes(
      selectAttributes(telemetry, {
        ...
        "gen_ai.usage.input_tokens": event.usage.inputTokens,
        "gen_ai.usage.output_tokens": event.usage.outputTokens,
        "gen_ai.usage.cache_read.input_tokens": (_b = event.usage.inputTokenDetails) == null ? void 0 : _b.cacheReadTokens,
    ...
    const finalToolOutputs = /* @__PURE__ */ new Map();
    for (const part of event.content) {
      if (part.type === "tool-error" || part.type === "tool-result" && part.preliminary !== true) {
        finalToolOutputs.set(part.toolCallId, part);
      }
    }
    const recordedToolCallIds = /* @__PURE__ */ new Set();
    for (const part of event.content) {
      if (part.type !== "tool-call" || !part.providerExecuted || recordedToolCallIds.has(part.toolCallId)) {
        continue;
      }
      ...
      if ((toolOutput == null ? void 0 : toolOutput.type) === "tool-result") {
        try {
      ...
      } else if ((toolOutput == null ? void 0 : toolOutput.type) === "tool-error") {
    ...
    state.inferenceSpan = void 0;
    state.inferenceContext = void 0;
    state.inferenceToolDefinitions = void 0;
  }
  onToolExecutionStart(event) {
    var _a;
    const state = this.getCallState(event.callId);
    const parentContext = (_a = state == null ? void 0 : state.stepContext) != null ? _a : state == null ? void 0 : state.rootContext;
...
var LegacyOpenTelemetry = class {
  ...
  onToolExecutionStart(event) {
    ...
    const attributes = selectAttributes2(telemetry, {
      ...
      ...Object.fromEntries(
        Object.entries(state.baseTelemetryAttributes).filter(
          ([key]) => key.startsWith("ai.settings.context.")
        )
      ),
