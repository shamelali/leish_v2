...
`));
        ...
        const message = typeof data === "object" && data != null && typeof data.message === "string" ? data.message : event.data;
        const type = (_b = (_a = event.eventType) != null ? _a : event.exceptionType) != null ? _b : "error";
...
Original error: ${errorMessage}`
  ...
  const createChatModel = (modelId) => new AnthropicLanguageModel(modelId, {
    ...
    transformRequestBody: (args, betas) => {
      ...
      const transformedTools = tools == null ? void 0 : tools.map(
        (rawTool) => {
          const { eager_input_streaming: eagerInputStreaming, ...tool } = rawTool;
